class User(object):
    def __init__(self, user_name=None, email=None, confirmed=0, admin=0, first_name=None, last_name=None, user_id=None, db_user=None):
        if db_user is not None:
            """ If creating user from db call unpack tuple """
            if isinstance(db_user, tuple):
                self.user_name = db_user[0]
                self.user_id = db_user[6]
                self.email = db_user[1]
                self.email_confirmed = db_user[2]
                self.admin = db_user[3]
                self.first_name = db_user[4]
                self.last_name = db_user[5]
            else:
                raise TypeError
        else:
